...
user_api_key = 'J02qcLb3NIgHS06Ow5CD5xEPXwyRuZjYBNPM4hVxf7pX01w3lcectGqlpPyMBcz5' # your disqus account api key
...
def main():
    ...
    for thread in threads:
        ...
        for comment in comments:
            ...
            iptag = comment.find('jskit:attribute', {'key' : 'IP'})
            if iptag:
                # find() succeeded, get value of ip address
                jskcom['ip'] = iptag['value']
            else:
                # find() returned None
                jskcom['ip'] = '255.255.255.0'    # arbitrary default value
            jskcom['name'] = comment.find('author')
            if jskcom['name']:
                jskcom['name'] = jskcom['name'].contents[0]
                jskcom['email'] = jskcom['name'].replace(' ','_')+'@jskit.hetnieuwewerkenblog.nl'
            else:
                jskcom['name'] = 'Guest'
                jskcom['email'] = 'guest@jskit.hetnieuwewerkenblog.nl'
            jskcom['url'] = None
        ...
        sort_by_date(jskcomments)

        # set up disqus thread
        thurl = 'http://hetnieuwewerkenblog.nl'+thid
        threq = None
        try:
