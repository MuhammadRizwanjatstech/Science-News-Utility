...
xhr.open("GET","http://news.sciencemag.org/rss/current.xml");
xhr.onload = function()
{
	try
	{
		...
		for (var c=0;c<items.length;c++)
		{
			...
			if (thumbnails && thumbnails.length > 0)
			{
                ...
                var description = item.getElementsByTagName("media:description").item(0).text;
                    newdescription = description.split("<"); //remove the link from the description                
				var row = Ti.UI.createTableViewRow({height:80});
				var label = Ti.UI.createLabel({
					text:title + " - " + newdescription[0],
					left:72,
					top:5,
					bottom:5,
					right:5,
